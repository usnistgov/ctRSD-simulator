# -*- coding: utf-8 -*-
"""
Created on Thu Jul 28 10:38:18 2022
...
for n in range(3):
    ...
    plt.plot(t-ts,DDn[:,n+3]*100,color=clp2[n],linewidth=2)
...
k_txn = [0.025,0.017,0.011] # 1/nM-s
...
for x in range(len(k_txn)):
    ...
    for n in range(len(IN)):
        ...
        model.global_rate_constants(ktxn=k_txn[x])
...
i = 0     
plt.subplot(3,5,1)
plt.plot(model.t/60,(REP2[i]/REP_con)*100,color=clp2[2],alpha=0.25,linewidth=1,linestyle='--')
plt.plot(model.t/60,(REP2[i+1]/REP_con)*100,color=clp2[2],linewidth=1,linestyle='--')
plt.plot(model.t/60,(REP2_l[i]/REP_con)*100,color=clp2[0],alpha=0.25,linewidth=1,linestyle='--')
plt.plot(model.t/60,(REP2_l[i+1]/REP_con)*100,color=clp2[0],linewidth=1,linestyle='--')
plt.plot(model.t/60,(REP2_h[i]/REP_con)*100,color=clp2[1],alpha=0.25,linewidth=1,linestyle='--')
plt.plot(model.t/60,(REP2_h[i+1]/REP_con)*100,color=clp2[1],linewidth=1,linestyle='--')
