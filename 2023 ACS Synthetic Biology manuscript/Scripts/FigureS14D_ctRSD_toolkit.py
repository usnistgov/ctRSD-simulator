# -*- coding: utf-8 -*-
"""
Created on Thu Jul 28 10:25:45 2022
...
ws = wb.sheet_by_name('Figure S14D')
...
plt.plot(t-ts,DDn[:,2]*100,color=clp[2],alpha=0.25,linewidth=2)
plt.plot(t-ts,DDn[:,3]*100,color=clp[3],alpha=0.25,linewidth=2)
plt.plot(t-ts,DDn[:,4]*100,color=clp[0],linewidth=2)
plt.plot(t-ts,DDn[:,5]*100,color=clp[1],linewidth=2)
plt.plot(t-ts,DDn[:,6]*100,color=clp[2],linewidth=2)
plt.plot(t-ts,DDn[:,7]*100,color=clp[3],linewidth=2)
...
krsd1 = [1e3/1e9,1e5/1e9,1e3/1e9,1e5/1e9] # 1/nM-s
krsd2 = [1e3/1e9,1e3/1e9,1e5/1e9,1e5/1e9] # 1/nM-s
...
for x in range(4):
    ...
    for n in range(len(IN)):
        ...
        model.molecular_species('G{5,1}',DNA_con=25,krsd=krsd2[x])
        model.molecular_species('G{1,2}',DNA_con=25,krsd=krsd1[x])
        ...
        plt.subplot(3,5,1)
        if  n == 0:
            plt.plot(model.t/60,(REP2/REP_con)*100,color=[0.3,0.3,0.3],linewidth=1,linestyle='--')
        else:
            plt.plot(model.t/60,(REP2/REP_con)*100,color=clp[x],linewidth=1,linestyle='--')
