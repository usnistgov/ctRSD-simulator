# -*- coding: utf-8 -*-
"""
Created on Thu Jul 28 09:27:24 2022
...
ws = wb.sheet_by_name('Figure S9B, middle-right')
...
for n in range(3):
    ...
    plt.plot(t-ts,DDn[:,n+6]*100,color=clp2[n],linewidth=2)
...
for n in range(3):
    plt.subplot(3,5,3)
    plt.plot(t-ts,DDn[:,n+3]*100,color=clp2[n+3],alpha=0.25,linewidth=2)
    plt.plot(t-ts,DDn[:,n+6+3]*100,color=clp2[n+3],linewidth=2)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.ylim(-10,110)
    plt.xlim(0,240)
